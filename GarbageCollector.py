...
scale = 0.4
kukaId = p.loadURDF("kuka/kuka_kr210_support/urdf/kr210l150.urdf", kukaCenter, kukaOrientation, globalScaling=scale)
...
def pick_garbage(position, object_id):
    """
...
def place_object_to_tray(trayPosition, kukaId, object_id):
    """
...
def main():
    ...
    print("\nAttempting to pick soupcan")
 
    pick_garbage(object_data.get('soupcan')['position'], object_data.get('soupcan')['id'])
    place_object_to_tray(trayPosition1,kukaId,object_data.get('soupcan')['id'])

    # Pick and place orange
    print("\nAttempting to pick banana")
    pick_garbage(object_data.get('orange')['position'], object_data.get('orange')['id'])
    place_object_to_tray(trayPosition2,kukaId,object_data.get('orange')['id'])

    # Pick and place tennisball
    print("\nAttempting to pick tennisball")
    pick_garbage(object_data.get('tennisball')['position'], object_data.get('tennisball')['id'])
    place_object_to_tray(trayPosition1,kukaId,object_data.get('tennisball')['id'])
